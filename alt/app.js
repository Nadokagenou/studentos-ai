...
function dayRail(sp, split, now) {
  ...
  // ตอนนี้มันมีก้อนของตัวเองอยู่เหนือราง (riskBlock) ที่ไม่ต้องแกล้งว่ามีเวลากำกับ
...
// ============================================================
// 1B38 — "เสี่ยงพัง": ของที่กำลังพังอยู่ตอนนี้
// ============================================================
// ก้อนนี้อยู่ตรงที่กริด "ทำอย่างอื่นต่อ" เคยอยู่ · สเปกรอบนี้ขอสามอย่างมาแทนกริด
// (งานถัดไปที่แนะนำ · คำแนะนำ AI · การเตือนความเสี่ยง) แต่สองอย่างแรกอยู่บนจอแล้วทั้งคู่:
// การ์ดฟ้าคือคำแนะนำ AI — มันเลือกงานใบนั้นให้เองจาก focusPlan() ไม่ใช่ผู้ใช้เลือก
// และรางเวลาก็คือรายการงานถัดไปที่เรียงตามเวลาอยู่แล้ว
// ใส่เพิ่มอีกสองก้อน = จอเดียวตอบคำถามเดิมสามรอบ ซึ่งคือของที่ 1B21 เพิ่งลบทิ้งไป
// (upNext() · remindersBlock() — ดูหมายเหตุใต้ dayRail)
//
// เหลือของใหม่จริงอย่างเดียวคือความเสี่ยง ซึ่งเดิมไม่มีที่อยู่บนหน้าแรกเลย
// ที่มาของรายชื่อจึงต่างจากรางโดยตั้งใจ: รางเอาเฉพาะ "ของที่ต้องจำ" ในกรอบ 48 ชม.
// (homeSplit + isRemindKind) ส่วนก้อนนี้เอาทุกใบที่เลยกำหนดไม่ว่าชนิดไหน —
// การบ้านที่เลยกำหนดมาอาทิตย์นึงไม่เคยโผล่บนหน้าแรกก่อนหน้านี้แม้แต่ครั้งเดียว
// ทั้งที่มันคือของที่เสียหายที่สุดในแอป
//
// เรื่องสี: พื้นเป็นการ์ดเหมือนราง ไม่ใช่แผ่นแดงเต็มก้อน — กฎ 60/30/10 ท้าย alt.css
// บอกว่าสีเตือนต้องเป็นของที่กินพื้นที่น้อยที่สุดบนจอ แดงจึงอยู่แค่ตัวเลขวันกับแถบข้างแถว
// แผ่นแดงเต็มความกว้างจะแย่งสายตาไปจากการ์ด "ตอนนี้" ซึ่งเป็นที่ที่การตัดสินใจจบจริง
function riskBlock(sp, now) {
  const shown = sp.now ? sp.now.task : null;
  const late = pendingTasks()
    .filter(t => t !== shown && t.due && new Date(t.due) < now)
    .sort((a, b) => new Date(a.due) - new Date(b.due));
  if (!late.length) return '';

  // สามใบคือจำนวนที่กวาดตาจบโดยไม่ต้องอ่านทีละบรรทัด — กฎเดียวกับ homeSplit
  // ที่เหลือไม่ได้หายไป มันมีบรรทัดของตัวเองที่พาไปแท็บ "งาน" ซึ่งลิสต์ครบอยู่แล้ว
  const top = late.slice(0, 3);
  const more = late.length - top.length;

  const rows = top.map(t => {
    const ms = now - new Date(t.due);
    const d = Math.floor(ms / 8.64e7);
    // ต่ำกว่าหนึ่งวันนับเป็นชั่วโมง — "0 วัน" ไม่ได้บอกอะไร และ "1 วัน" ก็ไม่จริง
    const num = d >= 1 ? d : Math.max(1, Math.floor(ms / 3.6e6));
    const unit = d >= 1 ? 'วัน' : 'ชม.';
    const sub = t.subject && t.subject !== 'อื่น ๆ' ? t.subject : typeInfo(t).name;
    return `<button class="rk-row" onclick="openForm('${t.id}')">
      <span class="rk-d"><b class="mono">${num}</b><i>${unit}</i></span>
      <span class="dr-b"><b>${esc(t.detail || t.subject || typeInfo(t).name)}</b>
        <span class="dr-m">เลยกำหนดมาแล้ว · ${esc(sub)}</span></span>
      <span class="dr-go">${icon('chevron')}</span>
    </button>`;
  }).join('');

  return `<section class="td-risk">
    <div class="rk-lb"><b>เสี่ยงพัง</b><span>${late.length} งานเลยกำหนด</span></div>
    ${rows}
    ${more ? `<button class="rk-more" onclick="go('scr-tasks')">
      ดูอีก ${more} งานที่เลยกำหนด${icon('chevron')}</button>` : ''}
  </section>`;
}
...
function renderMenu() {
  ...
  //   4 เสี่ยงพัง       อะไรกำลังพังอยู่ตอนนี้ — ของที่แพงที่สุดที่เคยไม่มีที่อยู่
  //   5 ราง            เย็นนี้เหลืออะไร ถึงกี่โมง
  //   6 เครื่องมือ      บรรทัดเดียวปิดท้าย ไม่ใช่กริดแปดก้อน
  //
  ...
  //
  // ข้อ 4 มาแทนที่กริด: ของสองในสามที่เคยคิดจะใส่ (งานถัดไป · คำแนะนำ AI) อยู่บนจอแล้ว
  // ทั้งคู่ — การ์ดฟ้าคือคำแนะนำ ส่วนรางคืองานถัดไป · ดู riskBlock() ว่าทำไมเหลือก้อนเดียว
