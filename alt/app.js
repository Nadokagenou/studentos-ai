...
const APP_VERSION = '1C23';                 // สายเลขของแอป
const APP_CODENAME = 'One Line';           // ชื่อรุ่นของอัปเดตนี้
...
// จอที่มีปุ่มของตัวเองอยู่บนแถบล่าง — ที่เหลือถือเป็นจอชั้นใน
// 1B93 · แถบล่างคือ วันนี้ · งาน · ＋ · น้องไซ · ฉัน (ห้าช่อง ปุ่ม + อยู่กลางเป๊ะ)
// scr-ai กลับขึ้นแถบล่างแล้ว ส่วน scr-mates เสียปุ่มไป — เข้าทางไทล์ "เพื่อนฉัน"
// ในกริดท้ายหน้าแรก กับลิงก์ในหน้าโปรไฟล์แทน (ดูเหตุผลด้านความปลอดภัยใน index.html)
// มันยังอยู่ในรายการนี้เพราะแถบล่างต้องโผล่ตอนอยู่ในฟีด ไม่งั้นออกจากจอนั้นไม่ได้
// scr-scan เข้าจากแผ่นปุ่ม + แทน — ยังนับเป็นจอในแอปเหมือนกัน
...
const TAB_OWNER = { 'scr-timeline': 'scr-tasks',
  ...
  // 1B93 · แท็บ "เพื่อน" ถูกแทนด้วยน้องไซ จอตระกูลฟีดจึงไม่มีแท็บของตัวเองอีกแล้ว
  // TAB_OWNER แปลงชั้นเดียว (tabId = TAB_OWNER[id] || id) การชี้มาที่ scr-mates
  // จึงไม่พอ — scr-mates เองก็ไม่ใช่แท็บแล้ว ต้องชี้ตรงไปที่ทางเข้าจริงคือ "วันนี้"
...
const LIVE_ONLY = {
  ...
  // 1B95b · สองจอนี้ตกสำรวจมาตลอด ทั้งที่วาดเนื้อในด้วย JS ทั้งใบเหมือนกันเป๊ะ
  // ผู้ใช้เจอจริง: กด "ดูแบบที่เพื่อนเห็น" จากแท็บ "ฉัน" แล้วกดย้อนกลับ ได้จอขาวล้วน
  // เพราะปุ่มนั้นพาไปฟีดที่ยังไม่เคยถูกวาด · ปุ่มถูกแก้ให้จำทางกลับแล้ว (userBack)
...
function go(id) {
  ...
  // 1C19 · scr-sai ถูกถอดออกจากรายการนี้ — เจ้าของสั่งว่า "อยากให้เป็นเหมือน
  // หน้าจอปกติแหละ" และหน้าจอปกติในแอปนี้มีแถบล่าง · ตอนเป็นสไลด์มันต้องซ่อน
...
//
// 1B94 · ตัวคั่นต้องหายไปด้วยเมื่อไม่มีรายละเอียด — ของเดิมต่อ ' · ' ทุกครั้งที่มีวิชา
// แล้วงานที่กรอกมาแต่ชื่อวิชา (ซึ่งเป็นงานที่แกะจากรูปเกือบทุกใบ) จะขึ้นว่า
// "คณิตศาสตร์ · " มีจุดค้างห้อยท้าย · เห็นชัดตอนทำจอ "งาน › วันนี้" เพราะการ์ดสีน้ำเงิน
// เอาชื่อนี้ไปขึ้นตัวใหญ่ 17px จุดที่ห้อยอยู่จึงกลายเป็นของที่เลี่ยงสายตาไม่ได้
// เจตนาเดิมของฟังก์ชันนี้เขียนไว้บรรทัดบนอยู่แล้วว่าไม่เอาตัวคั่นที่ไม่มีของตามหลัง
function taskTitle(t) {
  const detail = (t.detail || '').trim();
  const subj = t.subject && t.subject !== 'อื่น ๆ' ? esc(t.subject) : '';
  if (!detail) return subj || esc(t.detail || '');
  return (subj ? subj + ' · ' : '') + esc(detail);
}
...
function taskTitleText(t) {
  ...
  if (!detail) return subj;
  return (subj ? subj + ' · ' : '') + detail;
}
// ---------- สีประจำวิชา ----------
...
// ============================================================
// 1C12 — กริดฟีเจอร์กลับมาอยู่ท้ายหน้าแรก (จอ scr-tools ถูกถอดทิ้ง)
// ============================================================
// 1B38 เอากริดออกจากหน้าแรกไปแทรกเป็นแถวในจอโปรไฟล์ · 1B38b ย้ายมันไปอยู่จอของตัวเอง
// ที่มีทางเข้าเดียวคือบรรทัดจาง ๆ ว่า "ฟีเจอร์อื่น ๆ" ท้ายจอ
//
// เจ้าของสั่งถอดทั้งจอ (19 ก.ย. 2569): "เอาตรงฟีเจอร์อื่น ๆ ออก แล้วด้านในหัวข้อนั้น
// เอามาใส่หน้าแรก เหมือนเดิม" — ของแปดอย่างที่กวาดตาเห็นพร้อมกันในสองแถว
// ถูกซ่อนหลังบรรทัดที่ไม่มีใครกด เพื่อแก้ปัญหาเรื่องสัดส่วนสีซึ่งเป็นปัญหาของจอ ไม่ใช่ของกริด
//
// เหตุผลเดิมที่เคยย้ายมันออก (ไทล์แปดก้อนพื้นสีแบรนด์กินพื้นที่สีเน้นมากกว่าการ์ด "ตอนนี้")
// ยังจริงอยู่ แต่มันอยู่ล่างสุดของจอ ใต้ราง — สายตาถึงมันหลังการ์ด "ตอนนี้" ไปนานแล้ว
// ทางเข้าที่หาไม่เจอแพงกว่าสัดส่วนสีที่เพี้ยนไปท้ายจอ
function toolsGrid() {
...
// toolsLink() ถูกลบใน 1C12 พร้อมจอ scr-tools — กริดอยู่บนหน้าแรกเองแล้ว
// ไม่ต้องมีบรรทัดพาไป (ข้อความ sosText('toolsLink') ถูกถอดออกจาก control.js ด้วย)
// CSS .td-more ยังอยู่ใน today.css เผื่อต้องถอยกลับ
...
function renderMenu() {
  ...
  //   6 กริดฟีเจอร์    แปดก้อนปิดท้าย (1C12 · เคยเป็นบรรทัด "ฟีเจอร์อื่น ๆ")
  //
  ...
  return `<div class="as-grip"></div>
    ...
    <div class="as-heroes">
      ...
      <!-- 1B93 · ท่าที่สาม: ไม่รู้ว่าจะเพิ่มอะไร ให้ถามก่อน (เจ้าของขอปุ่ม AI ในแผ่นนี้)
           ...
           ไม่อยู่ใน ADD_ACTIONS เพราะอาเรย์นั้นถูกหั่นเป็น hero/rest ด้วย slice(0,2)
           การยัดเข้าไปจะดันท่า "พูดเพิ่มงาน" ตกลงไปเป็นรายการเงียบ ๆ ข้างล่าง
           ซึ่งเป็นท่าที่เร็วที่สุด (ห้าวินาที) — ห้ามให้มันเสียที่ไทล์ใหญ่ -->
...
// ============================================================
// 1B93 · สวิตช์สองโหมดบนหัวจองาน — เจ้าของสั่งเอง (13 ก.ย. 2569)
// ============================================================
// "หน้างานผมไม่ค่อยชอบ มันดูยาวแปลก ๆ อาจจะให้มีปฏิทิน หรือแค่มีงานเป็นข้อ ๆ
//  เพิ่มมา ถ้าใครอยากดูแค่งาน" — เจ้าของเลือกทางหลัง (ไม่เอาปฏิทินกลับ)
//
// **ทำไมสองโหมด ไม่ใช่สามเหมือนก่อน 1B47**
// ของเดิมคือ รายวัน/ปฏิทิน/รายการงาน ซึ่งถูกถอดออกเพราะสวิตช์สามช่อง + แถบวันเจ็ดช่อง
// + ช่องค้นหา กินจอเกือบครึ่งก่อนเห็นงานใบแรก · เหตุผลนั้นยังจริงอยู่ทุกคำ
// สองช่องกินสูงราว 34px แถวเดียว และไม่มีแถบวันกลับมาด้วย จึงไม่ได้พาปัญหาเดิมกลับมา
//
// **ทำไมโหมด "รายการ" ถึงไม่ใช่แค่สัปดาห์ที่ถอดหัววันออก**
// สัปดาห์ตอบ "วันไหนหนัก" — เรียงตามวัน งานด่วนของวันศุกร์จึงอยู่ล่างสุดเสมอ
// รายการตอบ "ทำอะไรก่อน" — เรียงตามความด่วนล้วน ข้ามวัน ไม่มีหัววันมาคั่น
// สองคำถามนี้ต่างกันจริง และนี่คือเหตุผลเดียวที่โหมดที่สองคุ้มที่จะมี
//
...
function renderTasks() {
  ...
  // rows มาจาก pendingTasks() ซึ่งเรียงตามความด่วนมาแล้ว — โหมด "รายการ" จึงใช้ลำดับเดิม
  // ไม่เรียงซ้ำ · เรียงใหม่ตรงนี้เท่ากับมีเกณฑ์ความด่วนสองชุดในแอปเดียว
  let listHTML;
  if (!rows.length) listHTML = '';
  else if (taskQ || taskView === 'list')
...
function renderContext() {
  ...
  if (ct) {
    ...
    // พูดอยู่แล้วด้วยการเป็นปุ่ม · ตั้งแต่ 1C10 แถวนี้เป็นคำชวนเดียวที่เหลือในจอนี้
    ct.textContent = know >= 100
      ? ctxClasses().length + ' คาบ · ' + ctxRoutines().length + ' กิจวัตร'
      : 'รู้จัก ' + know + '%';
  }
  if (row) row.hidden = false;
  // 1C10 · แถบ "รู้จักคุณแล้ว N% — AI เดาเวลาว่างอยู่" ถูกถอดออกจากแท็บ "ฉัน"
  // เจ้าของชี้มาเองว่า "เอาออก" (19 ก.ย. 2569)
  //
  // **ทางเข้ายังครบ ไม่ได้หายไปไหน** — แถว "บริบทของฉัน" ในลิสต์ข้างล่างพาไป
  // จอเดียวกัน และบอกค่าเดียวกันไว้บนแถว ("รู้จัก 0%" / "N คาบ · M กิจวัตร")
  // ซึ่งแปลว่าข้อมูลที่แถบนี้เคยบอกยังอ่านได้โดยไม่ต้องกดอะไรเลย เหมือนเดิมเป๊ะ
  // ที่หายไปคือ "เสียง" ของมัน ไม่ใช่ "ข้อมูล" หรือ "ทางไป"
  //
  // 1A9g เคยเถียงว่าคำชวนต้องดังกว่าแถวอื่นตอนที่ยังจำเป็น — ข้อนั้นยังถูกในหลักการ
  // แต่จอนี้มีคำชวนซ้อนกันสองชั้นอยู่แล้ว (แถบ + แถวในลิสต์ที่บอก % เหมือนกัน)
...
// มีทางเข้า wizOpen() อยู่สองที่ — แถว "บริบทของฉัน" ในลิสต์ และปุ่มนี้ (1C10 ถอดแถบออก)
...
function renderTabBadges() {
  // ---------- 1B93 · ไม่มีแท็บ "เพื่อน" บนแถบล่างแล้ว ----------
  // ช่องที่ 4 เป็นน้องไซตั้งแต่ 1B93 (ดูเหตุผลด้านความปลอดภัยใน index.html)
  // บล็อกที่เคยซ่อน #tabMates ตอน features.social ปิดจึงถูกถอดออก — ไม่มีปุ่มให้ซ่อน
  // และผลข้างเคียงเดิม "แถบเหลือสี่ช่อง ปุ่ม + เยื้องครึ่งช่อง" ก็หมดไปด้วย
  // ปิด features.social ตอนนี้กรองแค่ไทล์ "เพื่อนฉัน" ในกริดท้ายหน้าแรก ซึ่งพอแล้ว
  // เพราะนั่นคือทางเข้าฟีดที่เหลืออยู่ทางเดียว
...
function renderShowcase() {
  ...
  // ---------- 1C10 · แต่ละช่องมีสีประจำตัว (เจ้าของวงมาเอง 19 ก.ย. 2569: "ใส่สี") ----------
  // ของเดิมทุกใบที่มีของแล้วเป็นทองเหมือนกันหมด ซึ่งมาจากกฎเก่าที่ว่า "ทอง = ได้มาแล้วจริง"
  // กฎนั้นยังอยู่ แต่มันตอบคำถามผิดข้อสำหรับแถวนี้: สามใบนี้ *ได้มาแล้ว* เหมือนกันหมด
  // อยู่แล้วเวลามีของ สีทองจึงไม่ได้แยกอะไรเลย มันแค่ทำให้สามใบดูเป็นใบเดียวกัน
  //
  // สีที่ใส่คือ "สีประจำตัวของสิ่งนั้น" ไม่ใช่สีที่เข้ารหัสลำดับ — ซึ่งเป็นข้อที่ 1B98
  // เคยถอดสีออกจากลิสต์ทางเข้าเพราะมันเข้ารหัสลำดับจริง ๆ ("สีฟีเจอร์มันมั่วไป")
  // ห้อง = ฟ้า (ที่ที่เข้าไปได้) · เหรียญ = ทอง (ของที่ได้มา) · ต่อเนื่อง = ไฟ
  // ส่วนช่องที่สี่เป็นสีของธีมเองอยู่แล้ว · ทุกสีมาจากโทเคนที่ทุกธีมนิยามครบ
  //
  ...
  box.innerHTML = `<div class="shw-row shw-4">
      ...
      ${tile("go('scr-stats')", 'flame', stk > 1, 'ต่อเนื่อง',
        stk > 1 ? stk + ' วันติด' : (stk === 1 ? 'เริ่มวันนี้' : 'ยังไม่เริ่ม'), 'streak')}
      ${thTile}
    </div>`;
}
...
function renderAll() {
  ...
  renderWhy();
  renderProfile(); renderShowcase(); renderStats(); renderPlan(); renderFriends(); renderBadges();
