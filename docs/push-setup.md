...
> **SQL ย้ายไปอยู่ที่ `supabase/migrations/` แล้ว (23 ส.ค. 2569)**
> ก้อน SQL ที่เคยอยู่ในไฟล์นี้รันไม่ได้ ตรวจไม่ได้ และไม่มีลำดับ —
> ตอนนี้อยู่ในไฟล์ `.sql` จริงที่เรียงตามลำดับการรันแล้ว สั่งทีเดียวได้ด้วย
> `supabase db push` ส่วนที่เหลือในไฟล์นี้เป็นคำอธิบายว่าทำไมต้องมีและตั้งค่ายังไง
